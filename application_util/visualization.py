...
class Visualization(object):
    ...
    def draw_foot(self, tracks, IDnum):
        self.viewer.thickness = 3
        for track in tracks:
            if not track.is_confirmed() or track.time_since_update > 0:
                continue
            
            if track.track_id == IDnum:
                bbox = track.to_tlwh()
                foot_x, foot_y = bbox[0]+bbox[2]/2, bbox[1]+bbox[3]
                self.viewer.color = 0, 0, 255
                self.viewer.circle(
                    foot_x, foot_y, 3, label=str(track.track_id))
                
                


    def draw_target_trackers(self, tracks, IDnum):
        self.viewer.thickness = 2
        for track in tracks:
            if not track.is_confirmed() or track.time_since_update > 0:
                continue
